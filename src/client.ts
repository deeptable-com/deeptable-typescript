...
export class DeepTable {
   ...
   */
  constructor({
    baseURL = readEnv('DEEPTABLE_BASE_URL'),
    apiKey = readEnv('DEEPTABLE_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
    ...
    const options: ClientOptions = {
      apiKey,
      ...opts,
      baseURL: baseURL || `https://api.deeptable.com`,
    };
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('DEEPTABLE_LOG'), "process.env['DEEPTABLE_LOG']", this) ??
      defaultLogLevel;
    ...
    const customHeadersEnv = readEnv('DEEPTABLE_CUSTOM_HEADERS');
    if (customHeadersEnv) {
  ...
  #baseURLOverridden(): boolean {
    return this.baseURL !== 'https://api.deeptable.com';
  }
